export const SUPPORTED_LOCALES = [
    'en',
    'ar',
    'de',
    'es',
    'fr',
    'hi',
    'id',
    'it',
    'ja',
    'ko',
    'pt-br',
    'ru',
    'tr',
    'vi',
    'zh-cn',
    'zh-tw',
] as const;

export type SupportedLocale = typeof SUPPORTED_LOCALES[number];

type TranslationKey =
    'action.installOnRemote' |
    'action.notToday' |
    'action.openSettings' |
    'alert.dailyCostExceeded' |
    'alert.fastModeDetected' |
    'alert.fastModeOptimized' |
    'alert.limitExceeded' |
    'alert.scanningLocalHost' |
    'message.alertsResumed' |
    'message.alertsSnoozed' |
    'message.codexOptimizeApplied' |
    'message.codexOptimizeRemoved' |
    'message.summaryCopied' |
    'tooltip.allTime' |
    'tooltip.clickToTogglePeriod' |
    'tooltip.combinedTotal' |
    'tooltip.copySummary' |
    'tooltip.copySummaryTitle' |
    'tooltip.input' |
    'tooltip.limitResets' |
    'tooltip.bankedResets' |
    'tooltip.limitUsed' |
    'tooltip.limits' |
    'tooltip.logDirectoryNotFound' |
    'tooltip.model' |
    'tooltip.noUsageThisMonth' |
    'tooltip.optimize' |
    'tooltip.optimizeTitle' |
    'tooltip.output' |
    'tooltip.period' |
    'tooltip.rate' |
    'tooltip.rtkTitle' |
    'tooltip.saved' |
    'tooltip.scanningLocalHost' |
    'tooltip.settings' |
    'tooltip.settingsTitle' |
    'tooltip.thisMonth' |
    'tooltip.title' |
    'tooltip.today' |
    'tooltip.total' |
    'tooltip.updated';

type TranslationMessages = Record<TranslationKey, string>;

const MESSAGES: Record<SupportedLocale, TranslationMessages> = {
    en: {
        'action.installOnRemote': 'Install on {remote}',
        'action.notToday': 'Not Today',
        'action.openSettings': 'Open Settings',
        'alert.dailyCostExceeded': "otak-usage daily cost alert: today's total is {total}, above your daily alert threshold of {threshold}.",
        'alert.fastModeDetected': 'otak-usage: {provider} fast mode is on — usage is billed at premium fast-mode rates.',
        'alert.fastModeOptimized': 'Context optimization was enabled once to keep the fast-mode context compact.',
        'alert.limitExceeded': 'otak-usage limit alert: {provider} {window} usage is at {pct}%, above your {threshold}% alert threshold.',
        'alert.scanningLocalHost': 'otak-usage is running on this machine, so it counts local usage only — what you run on {remote} is not included.',
        'message.alertsResumed': 'otak-usage: alerts resumed',
        'message.alertsSnoozed': 'otak-usage: alerts silenced for the rest of today',
        'message.codexOptimizeApplied': 'otak-usage: applied Codex context optimization to config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: removed Codex context optimization from config.toml',
        'message.summaryCopied': 'otak-usage: summary copied to clipboard',
        'tooltip.allTime': 'All Time',
        'tooltip.clickToTogglePeriod': 'Click to switch view',
        'tooltip.combinedTotal': 'OpenAI + Claude Total',
        'tooltip.copySummary': 'Copy Summary',
        'tooltip.copySummaryTitle': 'Copy the usage summary to the clipboard',
        'tooltip.input': 'Input',
        'tooltip.limitResets': 'resets {time}',
        'tooltip.bankedResets': 'Banked resets',
        'tooltip.limitUsed': '{pct}% used',
        'tooltip.limits': 'Limits',
        'tooltip.logDirectoryNotFound': 'Log directory not found.',
        'tooltip.model': 'Model',
        'tooltip.noUsageThisMonth': 'No usage this month.',
        'tooltip.optimize': 'Optimize',
        'tooltip.optimizeTitle': 'Configure Claude Code and Codex context optimization',
        'tooltip.output': 'Output',
        'tooltip.period': 'Period',
        'tooltip.rate': 'Rate',
        'tooltip.rtkTitle': 'RTK — Token Savings',
        'tooltip.saved': 'Saved',
        'tooltip.scanningLocalHost': 'Reading this machine — usage on {remote} is not counted.',
        'tooltip.settings': 'Settings',
        'tooltip.settingsTitle': 'Open otak-usage settings',
        'tooltip.thisMonth': 'This Month',
        'tooltip.title': 'otak-usage — API-equivalent cost',
        'tooltip.today': 'Today',
        'tooltip.total': 'Total',
        'tooltip.updated': 'Updated',
    },
    ar: {
        'action.installOnRemote': 'التثبيت على {remote}',
        'action.notToday': 'ليس اليوم',
        'action.openSettings': 'فتح الإعدادات',
        'alert.dailyCostExceeded': 'تنبيه تكلفة otak-usage اليومية: إجمالي اليوم هو {total}، وقد تجاوز حد التنبيه اليومي {threshold}.',
        'alert.fastModeDetected': 'otak-usage: الوضع السريع لـ {provider} مفعّل — يُحتسب الاستخدام بأسعار الوضع السريع المميزة.',
        'alert.fastModeOptimized': 'تم تفعيل تحسين السياق مرة واحدة للحفاظ على سياق مضغوط في الوضع السريع.',
        'alert.limitExceeded': 'تنبيه حد otak-usage: استخدام {provider} {window} عند {pct}%، وهو أعلى من حد التنبيه البالغ {threshold}%.',
        'alert.scanningLocalHost': 'يعمل otak-usage على هذا الجهاز، لذا يحتسب الاستخدام المحلي فقط — وما تشغّله على {remote} غير مشمول.',
        'message.alertsResumed': 'otak-usage: تم استئناف التنبيهات',
        'message.alertsSnoozed': 'otak-usage: تم كتم التنبيهات لبقية اليوم',
        'message.codexOptimizeApplied': 'otak-usage: تم تطبيق تحسين سياق Codex على config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: تمت إزالة تحسين سياق Codex من config.toml',
        'message.summaryCopied': 'otak-usage: تم نسخ الملخص إلى الحافظة',
        'tooltip.allTime': 'كل الأوقات',
        'tooltip.clickToTogglePeriod': 'انقر لتبديل العرض',
        'tooltip.combinedTotal': 'إجمالي OpenAI + Claude',
        'tooltip.copySummary': 'نسخ الملخص',
        'tooltip.copySummaryTitle': 'نسخ ملخص الاستخدام إلى الحافظة',
        'tooltip.input': 'الإدخال',
        'tooltip.limitResets': 'يُعاد الضبط {time}',
        'tooltip.bankedResets': 'عمليات إعادة التعيين المخزّنة',
        'tooltip.limitUsed': '{pct}% مستخدم',
        'tooltip.limits': 'الحدود',
        'tooltip.logDirectoryNotFound': 'لم يتم العثور على مجلد السجلات.',
        'tooltip.model': 'النموذج',
        'tooltip.noUsageThisMonth': 'لا يوجد استخدام هذا الشهر.',
        'tooltip.optimize': 'تحسين',
        'tooltip.optimizeTitle': 'تكوين تحسين السياق لـ Claude Code وCodex',
        'tooltip.output': 'الإخراج',
        'tooltip.period': 'الفترة',
        'tooltip.rate': 'النسبة',
        'tooltip.rtkTitle': 'RTK — توفير الرموز',
        'tooltip.saved': 'المحفوظ',
        'tooltip.scanningLocalHost': 'تتم القراءة من هذا الجهاز — لا يُحتسب الاستخدام على {remote}.',
        'tooltip.settings': 'الإعدادات',
        'tooltip.settingsTitle': 'فتح إعدادات otak-usage',
        'tooltip.thisMonth': 'هذا الشهر',
        'tooltip.title': 'otak-usage — تكلفة مكافئة لواجهة API',
        'tooltip.today': 'اليوم',
        'tooltip.total': 'الإجمالي',
        'tooltip.updated': 'تم التحديث',
    },
    de: {
        'action.installOnRemote': 'Auf {remote} installieren',
        'action.notToday': 'Heute nicht mehr',
        'action.openSettings': 'Einstellungen öffnen',
        'alert.dailyCostExceeded': 'otak-usage Tageskostenalarm: Die heutige Summe beträgt {total} und liegt über dem täglichen Grenzwert von {threshold}.',
        'alert.fastModeDetected': 'otak-usage: Der Fast-Modus von {provider} ist aktiv — die Nutzung wird zu den höheren Fast-Modus-Preisen abgerechnet.',
        'alert.fastModeOptimized': 'Die Kontextoptimierung wurde einmalig aktiviert, um den Kontext im Fast-Modus kompakt zu halten.',
        'alert.limitExceeded': 'otak-usage-Limit-Alarm: Die Auslastung von {provider} {window} liegt bei {pct}% und damit über Ihrem Alarmschwellenwert von {threshold}%.',
        'alert.scanningLocalHost': 'otak-usage läuft auf diesem Rechner und zählt daher nur die lokale Nutzung — was Sie auf {remote} ausführen, ist nicht enthalten.',
        'message.alertsResumed': 'otak-usage: Alarme wieder aktiviert',
        'message.alertsSnoozed': 'otak-usage: Alarme für den Rest des Tages stummgeschaltet',
        'message.codexOptimizeApplied': 'otak-usage: Codex-Kontextoptimierung auf config.toml angewendet',
        'message.codexOptimizeRemoved': 'otak-usage: Codex-Kontextoptimierung aus config.toml entfernt',
        'message.summaryCopied': 'otak-usage: Zusammenfassung in die Zwischenablage kopiert',
        'tooltip.allTime': 'Gesamtzeit',
        'tooltip.clickToTogglePeriod': 'Klicken, um die Ansicht zu wechseln',
        'tooltip.combinedTotal': 'OpenAI + Claude Gesamt',
        'tooltip.copySummary': 'Zusammenfassung kopieren',
        'tooltip.copySummaryTitle': 'Nutzungszusammenfassung in die Zwischenablage kopieren',
        'tooltip.input': 'Eingabe',
        'tooltip.limitResets': 'Reset {time}',
        'tooltip.bankedResets': 'Banked Resets',
        'tooltip.limitUsed': '{pct}% verbraucht',
        'tooltip.limits': 'Limits',
        'tooltip.logDirectoryNotFound': 'Protokollverzeichnis nicht gefunden.',
        'tooltip.model': 'Modell',
        'tooltip.noUsageThisMonth': 'Keine Nutzung in diesem Monat.',
        'tooltip.optimize': 'Optimieren',
        'tooltip.optimizeTitle': 'Kontextoptimierung für Claude Code und Codex konfigurieren',
        'tooltip.output': 'Ausgabe',
        'tooltip.period': 'Zeitraum',
        'tooltip.rate': 'Rate',
        'tooltip.rtkTitle': 'RTK — Token-Ersparnis',
        'tooltip.saved': 'Gespart',
        'tooltip.scanningLocalHost': 'Liest diesen Rechner — die Nutzung auf {remote} wird nicht gezählt.',
        'tooltip.settings': 'Einstellungen',
        'tooltip.settingsTitle': 'otak-usage-Einstellungen öffnen',
        'tooltip.thisMonth': 'Dieser Monat',
        'tooltip.title': 'otak-usage — API-äquivalente Kosten',
        'tooltip.today': 'Heute',
        'tooltip.total': 'Gesamt',
        'tooltip.updated': 'Aktualisiert',
    },
    es: {
        'action.installOnRemote': 'Instalar en {remote}',
        'action.notToday': 'Hoy no',
        'action.openSettings': 'Abrir configuración',
        'alert.dailyCostExceeded': 'Alerta de coste diario de otak-usage: el total de hoy es {total}, por encima del umbral diario de {threshold}.',
        'alert.fastModeDetected': 'otak-usage: el modo rápido de {provider} está activo — el uso se factura con las tarifas premium del modo rápido.',
        'alert.fastModeOptimized': 'La optimización de contexto se activó una única vez para mantener compacto el contexto en modo rápido.',
        'alert.limitExceeded': 'Alerta de límite de otak-usage: el uso de {provider} {window} está en {pct}%, por encima de tu umbral de alerta del {threshold}%.',
        'alert.scanningLocalHost': 'otak-usage se ejecuta en este equipo, así que solo cuenta el uso local — lo que ejecutas en {remote} no se incluye.',
        'message.alertsResumed': 'otak-usage: alertas reanudadas',
        'message.alertsSnoozed': 'otak-usage: alertas silenciadas por el resto del día',
        'message.codexOptimizeApplied': 'otak-usage: optimización de contexto de Codex aplicada a config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: optimización de contexto de Codex eliminada de config.toml',
        'message.summaryCopied': 'otak-usage: resumen copiado al portapapeles',
        'tooltip.allTime': 'Todo el tiempo',
        'tooltip.clickToTogglePeriod': 'Haz clic para cambiar la vista',
        'tooltip.combinedTotal': 'Total de OpenAI + Claude',
        'tooltip.copySummary': 'Copiar resumen',
        'tooltip.copySummaryTitle': 'Copiar el resumen de uso al portapapeles',
        'tooltip.input': 'Entrada',
        'tooltip.limitResets': 'se reinicia {time}',
        'tooltip.bankedResets': 'Restablecimientos guardados',
        'tooltip.limitUsed': '{pct}% usado',
        'tooltip.limits': 'Límites',
        'tooltip.logDirectoryNotFound': 'No se encontró el directorio de registros.',
        'tooltip.model': 'Modelo',
        'tooltip.noUsageThisMonth': 'Sin uso este mes.',
        'tooltip.optimize': 'Optimizar',
        'tooltip.optimizeTitle': 'Configurar la optimización de contexto de Claude Code y Codex',
        'tooltip.output': 'Salida',
        'tooltip.period': 'Periodo',
        'tooltip.rate': 'Tasa',
        'tooltip.rtkTitle': 'RTK — Ahorro de tokens',
        'tooltip.saved': 'Ahorrado',
        'tooltip.scanningLocalHost': 'Lee este equipo — el uso en {remote} no se cuenta.',
        'tooltip.settings': 'Configuración',
        'tooltip.settingsTitle': 'Abrir la configuración de otak-usage',
        'tooltip.thisMonth': 'Este mes',
        'tooltip.title': 'otak-usage — coste equivalente de API',
        'tooltip.today': 'Hoy',
        'tooltip.total': 'Total',
        'tooltip.updated': 'Actualizado',
    },
    fr: {
        'action.installOnRemote': 'Installer sur {remote}',
        'action.notToday': "Pas aujourd'hui",
        'action.openSettings': 'Ouvrir les paramètres',
        'alert.dailyCostExceeded': "Alerte de coût quotidien otak-usage : le total d'aujourd'hui est de {total}, au-dessus du seuil quotidien de {threshold}.",
        'alert.fastModeDetected': "otak-usage : le mode rapide de {provider} est actif — l'utilisation est facturée aux tarifs majorés du mode rapide.",
        'alert.fastModeOptimized': "L'optimisation du contexte a été activée une seule fois pour garder un contexte compact en mode rapide.",
        'alert.limitExceeded': "Alerte de limite otak-usage : l'utilisation de {provider} {window} est à {pct}%, au-dessus de votre seuil d'alerte de {threshold}%.",
        'alert.scanningLocalHost': "otak-usage s'exécute sur cette machine et ne compte donc que l'utilisation locale — ce que vous exécutez sur {remote} n'est pas inclus.",
        'message.alertsResumed': 'otak-usage : alertes réactivées',
        'message.alertsSnoozed': 'otak-usage : alertes désactivées pour le reste de la journée',
        'message.codexOptimizeApplied': 'otak-usage : optimisation du contexte Codex appliquée à config.toml',
        'message.codexOptimizeRemoved': 'otak-usage : optimisation du contexte Codex supprimée de config.toml',
        'message.summaryCopied': 'otak-usage : résumé copié dans le presse-papiers',
        'tooltip.allTime': 'Depuis toujours',
        'tooltip.clickToTogglePeriod': 'Cliquez pour changer de vue',
        'tooltip.combinedTotal': 'Total OpenAI + Claude',
        'tooltip.copySummary': 'Copier le résumé',
        'tooltip.copySummaryTitle': "Copier le résumé d'utilisation dans le presse-papiers",
        'tooltip.input': 'Entrée',
        'tooltip.limitResets': 'réinitialisation {time}',
        'tooltip.bankedResets': 'Réinitialisations mises de côté',
        'tooltip.limitUsed': '{pct}% utilisé',
        'tooltip.limits': 'Limites',
        'tooltip.logDirectoryNotFound': 'Répertoire des journaux introuvable.',
        'tooltip.model': 'Modèle',
        'tooltip.noUsageThisMonth': 'Aucune utilisation ce mois-ci.',
        'tooltip.optimize': 'Optimiser',
        'tooltip.optimizeTitle': "Configurer l'optimisation du contexte de Claude Code et Codex",
        'tooltip.output': 'Sortie',
        'tooltip.period': 'Période',
        'tooltip.rate': 'Taux',
        'tooltip.rtkTitle': 'RTK — Économies de tokens',
        'tooltip.saved': 'Économisé',
        'tooltip.scanningLocalHost': "Lit cette machine — l'utilisation sur {remote} n'est pas comptée.",
        'tooltip.settings': 'Paramètres',
        'tooltip.settingsTitle': 'Ouvrir les paramètres otak-usage',
        'tooltip.thisMonth': 'Ce mois-ci',
        'tooltip.title': 'otak-usage — coût équivalent API',
        'tooltip.today': "Aujourd'hui",
        'tooltip.total': 'Total',
        'tooltip.updated': 'Mis à jour',
    },
    hi: {
        'action.installOnRemote': '{remote} पर इंस्टॉल करें',
        'action.notToday': 'आज नहीं',
        'action.openSettings': 'सेटिंग खोलें',
        'alert.dailyCostExceeded': 'otak-usage दैनिक लागत अलर्ट: आज का कुल {total} है, जो आपके दैनिक अलर्ट सीमा {threshold} से अधिक है।',
        'alert.fastModeDetected': 'otak-usage: {provider} का फ़ास्ट मोड चालू है — उपयोग प्रीमियम फ़ास्ट-मोड दरों पर बिल किया जाता है।',
        'alert.fastModeOptimized': 'फ़ास्ट मोड में कॉन्टेक्स्ट को संक्षिप्त रखने के लिए कॉन्टेक्स्ट ऑप्टिमाइज़ेशन एक बार सक्षम किया गया।',
        'alert.limitExceeded': 'otak-usage सीमा अलर्ट: {provider} {window} उपयोग {pct}% है, जो आपकी {threshold}% अलर्ट सीमा से अधिक है।',
        'alert.scanningLocalHost': 'otak-usage इसी मशीन पर चल रहा है, इसलिए यह केवल स्थानीय उपयोग गिनता है — {remote} पर आप जो चलाते हैं वह शामिल नहीं है।',
        'message.alertsResumed': 'otak-usage: alert फिर से चालू कर दिए गए',
        'message.alertsSnoozed': 'otak-usage: आज के बाकी समय के लिए alert बंद कर दिए गए',
        'message.codexOptimizeApplied': 'otak-usage: Codex संदर्भ अनुकूलन config.toml में लागू किया गया',
        'message.codexOptimizeRemoved': 'otak-usage: Codex संदर्भ अनुकूलन config.toml से हटाया गया',
        'message.summaryCopied': 'otak-usage: सारांश क्लिपबोर्ड पर कॉपी किया गया',
        'tooltip.allTime': 'अब तक',
        'tooltip.clickToTogglePeriod': 'दृश्य बदलने के लिए क्लिक करें',
        'tooltip.combinedTotal': 'OpenAI + Claude कुल',
        'tooltip.copySummary': 'सारांश कॉपी करें',
        'tooltip.copySummaryTitle': 'उपयोग सारांश क्लिपबोर्ड पर कॉपी करें',
        'tooltip.input': 'इनपुट',
        'tooltip.limitResets': '{time} पर रीसेट',
        'tooltip.bankedResets': 'बैंक किए गए रीसेट',
        'tooltip.limitUsed': '{pct}% उपयोग',
        'tooltip.limits': 'सीमाएँ',
        'tooltip.logDirectoryNotFound': 'लॉग डायरेक्टरी नहीं मिली।',
        'tooltip.model': 'मॉडल',
        'tooltip.noUsageThisMonth': 'इस महीने कोई उपयोग नहीं।',
        'tooltip.optimize': 'अनुकूलित करें',
        'tooltip.optimizeTitle': 'Claude Code और Codex संदर्भ अनुकूलन कॉन्फ़िगर करें',
        'tooltip.output': 'आउटपुट',
        'tooltip.period': 'अवधि',
        'tooltip.rate': 'दर',
        'tooltip.rtkTitle': 'RTK — टोकन बचत',
        'tooltip.saved': 'बचत',
        'tooltip.scanningLocalHost': 'यह मशीन पढ़ी जा रही है — {remote} का उपयोग नहीं गिना जाता।',
        'tooltip.settings': 'सेटिंग',
        'tooltip.settingsTitle': 'otak-usage सेटिंग खोलें',
        'tooltip.thisMonth': 'इस महीने',
        'tooltip.title': 'otak-usage — API-समतुल्य लागत',
        'tooltip.today': 'आज',
        'tooltip.total': 'कुल',
        'tooltip.updated': 'अपडेट',
    },
    id: {
        'action.installOnRemote': 'Pasang di {remote}',
        'action.notToday': 'Jangan hari ini',
        'action.openSettings': 'Buka Pengaturan',
        'alert.dailyCostExceeded': 'Peringatan biaya harian otak-usage: total hari ini {total}, melebihi ambang peringatan harian {threshold}.',
        'alert.fastModeDetected': 'otak-usage: mode cepat {provider} aktif — penggunaan ditagih dengan tarif premium mode cepat.',
        'alert.fastModeOptimized': 'Optimalisasi konteks diaktifkan satu kali untuk menjaga konteks mode cepat tetap ringkas.',
        'alert.limitExceeded': 'Peringatan batas otak-usage: penggunaan {provider} {window} berada di {pct}%, melebihi ambang peringatan {threshold}% Anda.',
        'alert.scanningLocalHost': 'otak-usage berjalan di mesin ini, jadi hanya menghitung penggunaan lokal — yang Anda jalankan di {remote} tidak termasuk.',
        'message.alertsResumed': 'otak-usage: peringatan diaktifkan kembali',
        'message.alertsSnoozed': 'otak-usage: peringatan dibisukan untuk sisa hari ini',
        'message.codexOptimizeApplied': 'otak-usage: pengoptimalan konteks Codex diterapkan ke config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: pengoptimalan konteks Codex dihapus dari config.toml',
        'message.summaryCopied': 'otak-usage: ringkasan disalin ke clipboard',
        'tooltip.allTime': 'Sepanjang Waktu',
        'tooltip.clickToTogglePeriod': 'Klik untuk mengganti tampilan',
        'tooltip.combinedTotal': 'Total OpenAI + Claude',
        'tooltip.copySummary': 'Salin Ringkasan',
        'tooltip.copySummaryTitle': 'Salin ringkasan penggunaan ke clipboard',
        'tooltip.input': 'Input',
        'tooltip.limitResets': 'reset {time}',
        'tooltip.bankedResets': 'Reset tersimpan',
        'tooltip.limitUsed': '{pct}% terpakai',
        'tooltip.limits': 'Batas',
        'tooltip.logDirectoryNotFound': 'Direktori log tidak ditemukan.',
        'tooltip.model': 'Model',
        'tooltip.noUsageThisMonth': 'Tidak ada penggunaan bulan ini.',
        'tooltip.optimize': 'Optimalkan',
        'tooltip.optimizeTitle': 'Konfigurasikan pengoptimalan konteks Claude Code dan Codex',
        'tooltip.output': 'Output',
        'tooltip.period': 'Periode',
        'tooltip.rate': 'Rasio',
        'tooltip.rtkTitle': 'RTK — Penghematan Token',
        'tooltip.saved': 'Dihemat',
        'tooltip.scanningLocalHost': 'Membaca mesin ini — penggunaan di {remote} tidak dihitung.',
        'tooltip.settings': 'Pengaturan',
        'tooltip.settingsTitle': 'Buka pengaturan otak-usage',
        'tooltip.thisMonth': 'Bulan Ini',
        'tooltip.title': 'otak-usage — biaya setara API',
        'tooltip.today': 'Hari Ini',
        'tooltip.total': 'Total',
        'tooltip.updated': 'Diperbarui',
    },
    it: {
        'action.installOnRemote': 'Installa su {remote}',
        'action.notToday': 'Non oggi',
        'action.openSettings': 'Apri impostazioni',
        'alert.dailyCostExceeded': 'Avviso costo giornaliero di otak-usage: il totale di oggi è {total}, sopra la soglia giornaliera di {threshold}.',
        'alert.fastModeDetected': "otak-usage: la modalità veloce di {provider} è attiva — l'utilizzo viene fatturato alle tariffe maggiorate della modalità veloce.",
        'alert.fastModeOptimized': "L'ottimizzazione del contesto è stata attivata una sola volta per mantenere compatto il contesto in modalità veloce.",
        'alert.limitExceeded': "Avviso limite di otak-usage: l'utilizzo di {provider} {window} è al {pct}%, sopra la soglia di avviso del {threshold}%.",
        'alert.scanningLocalHost': "otak-usage è in esecuzione su questo computer, quindi conta solo l'utilizzo locale — ciò che esegui su {remote} non è incluso.",
        'message.alertsResumed': 'otak-usage: avvisi riattivati',
        'message.alertsSnoozed': 'otak-usage: avvisi silenziati per il resto della giornata',
        'message.codexOptimizeApplied': 'otak-usage: ottimizzazione del contesto Codex applicata a config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: ottimizzazione del contesto Codex rimossa da config.toml',
        'message.summaryCopied': 'otak-usage: riepilogo copiato negli appunti',
        'tooltip.allTime': 'Da sempre',
        'tooltip.clickToTogglePeriod': 'Fai clic per cambiare vista',
        'tooltip.combinedTotal': 'Totale OpenAI + Claude',
        'tooltip.copySummary': 'Copia riepilogo',
        'tooltip.copySummaryTitle': 'Copia il riepilogo di utilizzo negli appunti',
        'tooltip.input': 'Input',
        'tooltip.limitResets': 'reset {time}',
        'tooltip.bankedResets': 'Reset accumulati',
        'tooltip.limitUsed': '{pct}% usato',
        'tooltip.limits': 'Limiti',
        'tooltip.logDirectoryNotFound': 'Directory dei log non trovata.',
        'tooltip.model': 'Modello',
        'tooltip.noUsageThisMonth': 'Nessun utilizzo questo mese.',
        'tooltip.optimize': 'Ottimizza',
        'tooltip.optimizeTitle': "Configura l'ottimizzazione del contesto di Claude Code e Codex",
        'tooltip.output': 'Output',
        'tooltip.period': 'Periodo',
        'tooltip.rate': 'Tasso',
        'tooltip.rtkTitle': 'RTK — Risparmio token',
        'tooltip.saved': 'Risparmiati',
        'tooltip.scanningLocalHost': "Legge questo computer — l'utilizzo su {remote} non viene conteggiato.",
        'tooltip.settings': 'Impostazioni',
        'tooltip.settingsTitle': 'Apri le impostazioni di otak-usage',
        'tooltip.thisMonth': 'Questo mese',
        'tooltip.title': 'otak-usage — costo equivalente API',
        'tooltip.today': 'Oggi',
        'tooltip.total': 'Totale',
        'tooltip.updated': 'Aggiornato',
    },
    ja: {
        'action.installOnRemote': '{remote} にインストール',
        'action.notToday': '今日はもう通知しない',
        'action.openSettings': '設定を開く',
        'alert.dailyCostExceeded': 'otak-usage の本日合計が {total} になり、1日あたりのアラートしきい値 {threshold} を超えました。',
        'alert.fastModeDetected': 'otak-usage: {provider} の fast モードが有効です — 使用量は割高な fast モード料金で課金されます。',
        'alert.fastModeOptimized': 'fast モードのコンテキストを小さく保つため、コンテキスト最適化を一度だけ有効化しました。',
        'alert.limitExceeded': 'otak-usage の {provider} {window} 使用率が {pct}% になり、アラートしきい値 {threshold}% を超えました。',
        'alert.scanningLocalHost': 'otak-usage はこのマシン上で動作しているため、ローカルの使用量だけを集計します — {remote} 上での使用量は含まれません。',
        'message.alertsResumed': 'otak-usage: アラートを再開しました',
        'message.alertsSnoozed': 'otak-usage: 本日はこれ以上アラートを通知しません',
        'message.codexOptimizeApplied': 'otak-usage: Codex コンテキスト最適化を config.toml に適用しました',
        'message.codexOptimizeRemoved': 'otak-usage: Codex コンテキスト最適化を config.toml から削除しました',
        'message.summaryCopied': 'otak-usage: サマリーをクリップボードにコピーしました',
        'tooltip.allTime': '全期間',
        'tooltip.clickToTogglePeriod': 'クリックして表示を切り替え',
        'tooltip.combinedTotal': 'OpenAI + Claude 合計',
        'tooltip.copySummary': 'サマリーをコピー',
        'tooltip.copySummaryTitle': '使用量サマリーをクリップボードにコピー',
        'tooltip.input': '入力',
        'tooltip.limitResets': '{time} リセット',
        'tooltip.bankedResets': 'バンク済みリセット',
        'tooltip.limitUsed': '{pct}% 使用',
        'tooltip.limits': '制限',
        'tooltip.logDirectoryNotFound': 'ログディレクトリが見つかりません。',
        'tooltip.model': 'モデル',
        'tooltip.noUsageThisMonth': '今月の使用量はありません。',
        'tooltip.optimize': '最適化',
        'tooltip.optimizeTitle': 'Claude Code と Codex のコンテキスト最適化を設定します',
        'tooltip.output': '出力',
        'tooltip.period': '期間',
        'tooltip.rate': '率',
        'tooltip.rtkTitle': 'RTK — トークン節約量',
        'tooltip.saved': '節約',
        'tooltip.scanningLocalHost': 'このマシンを参照中 — {remote} の使用量は集計されません。',
        'tooltip.settings': '設定',
        'tooltip.settingsTitle': 'otak-usage の設定を開く',
        'tooltip.thisMonth': '今月',
        'tooltip.title': 'otak-usage — API 相当コスト',
        'tooltip.today': '本日',
        'tooltip.total': '合計',
        'tooltip.updated': '更新',
    },
    ko: {
        'action.installOnRemote': '{remote}에 설치',
        'action.notToday': '오늘은 그만',
        'action.openSettings': '설정 열기',
        'alert.dailyCostExceeded': 'otak-usage 일일 비용 알림: 오늘 합계가 {total}이며 일일 알림 기준 {threshold}를 초과했습니다.',
        'alert.fastModeDetected': 'otak-usage: {provider}의 fast 모드가 켜져 있습니다 — 사용량은 프리미엄 fast 모드 요금으로 청구됩니다.',
        'alert.fastModeOptimized': 'fast 모드의 컨텍스트를 간결하게 유지하도록 컨텍스트 최적화를 한 번만 활성화했습니다.',
        'alert.limitExceeded': 'otak-usage 한도 알림: {provider} {window} 사용량이 {pct}%로 알림 기준 {threshold}%를 초과했습니다.',
        'alert.scanningLocalHost': 'otak-usage가 이 컴퓨터에서 실행 중이므로 로컬 사용량만 집계합니다 — {remote}에서 실행한 사용량은 포함되지 않습니다.',
        'message.alertsResumed': 'otak-usage: 알림을 다시 시작했습니다',
        'message.alertsSnoozed': 'otak-usage: 오늘 남은 시간 동안 알림을 표시하지 않습니다',
        'message.codexOptimizeApplied': 'otak-usage: Codex 컨텍스트 최적화를 config.toml에 적용했습니다',
        'message.codexOptimizeRemoved': 'otak-usage: Codex 컨텍스트 최적화를 config.toml에서 제거했습니다',
        'message.summaryCopied': 'otak-usage: 요약을 클립보드에 복사했습니다',
        'tooltip.allTime': '전체 기간',
        'tooltip.clickToTogglePeriod': '클릭하여 보기 전환',
        'tooltip.combinedTotal': 'OpenAI + Claude 합계',
        'tooltip.copySummary': '요약 복사',
        'tooltip.copySummaryTitle': '사용량 요약을 클립보드에 복사',
        'tooltip.input': '입력',
        'tooltip.limitResets': '{time} 초기화',
        'tooltip.bankedResets': '저장된 리셋',
        'tooltip.limitUsed': '{pct}% 사용',
        'tooltip.limits': '한도',
        'tooltip.logDirectoryNotFound': '로그 디렉터리를 찾을 수 없습니다.',
        'tooltip.model': '모델',
        'tooltip.noUsageThisMonth': '이번 달 사용량이 없습니다.',
        'tooltip.optimize': '최적화',
        'tooltip.optimizeTitle': 'Claude Code와 Codex 컨텍스트 최적화 구성',
        'tooltip.output': '출력',
        'tooltip.period': '기간',
        'tooltip.rate': '비율',
        'tooltip.rtkTitle': 'RTK — 토큰 절약',
        'tooltip.saved': '절약',
        'tooltip.scanningLocalHost': '이 컴퓨터를 읽는 중 — {remote}의 사용량은 집계되지 않습니다.',
        'tooltip.settings': '설정',
        'tooltip.settingsTitle': 'otak-usage 설정 열기',
        'tooltip.thisMonth': '이번 달',
        'tooltip.title': 'otak-usage — API 상당 비용',
        'tooltip.today': '오늘',
        'tooltip.total': '합계',
        'tooltip.updated': '업데이트',
    },
    'pt-br': {
        'action.installOnRemote': 'Instalar no {remote}',
        'action.notToday': 'Hoje não',
        'action.openSettings': 'Abrir configurações',
        'alert.dailyCostExceeded': 'Alerta de custo diário do otak-usage: o total de hoje é {total}, acima do limite diário de {threshold}.',
        'alert.fastModeDetected': 'otak-usage: o modo rápido do {provider} está ativo — o uso é cobrado com as tarifas premium do modo rápido.',
        'alert.fastModeOptimized': 'A otimização de contexto foi ativada uma única vez para manter o contexto compacto no modo rápido.',
        'alert.limitExceeded': 'Alerta de limite do otak-usage: o uso de {provider} {window} está em {pct}%, acima do seu limite de alerta de {threshold}%.',
        'alert.scanningLocalHost': 'O otak-usage está sendo executado nesta máquina, portanto conta apenas o uso local — o que você executa no {remote} não é incluído.',
        'message.alertsResumed': 'otak-usage: alertas retomados',
        'message.alertsSnoozed': 'otak-usage: alertas silenciados pelo resto do dia',
        'message.codexOptimizeApplied': 'otak-usage: otimização de contexto do Codex aplicada ao config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: otimização de contexto do Codex removida do config.toml',
        'message.summaryCopied': 'otak-usage: resumo copiado para a área de transferência',
        'tooltip.allTime': 'Todo o período',
        'tooltip.clickToTogglePeriod': 'Clique para alternar a visualização',
        'tooltip.combinedTotal': 'Total OpenAI + Claude',
        'tooltip.copySummary': 'Copiar resumo',
        'tooltip.copySummaryTitle': 'Copiar o resumo de uso para a área de transferência',
        'tooltip.input': 'Entrada',
        'tooltip.limitResets': 'redefine {time}',
        'tooltip.bankedResets': 'Redefinições reservadas',
        'tooltip.limitUsed': '{pct}% usado',
        'tooltip.limits': 'Limites',
        'tooltip.logDirectoryNotFound': 'Diretório de logs não encontrado.',
        'tooltip.model': 'Modelo',
        'tooltip.noUsageThisMonth': 'Sem uso neste mês.',
        'tooltip.optimize': 'Otimizar',
        'tooltip.optimizeTitle': 'Configurar a otimização de contexto do Claude Code e Codex',
        'tooltip.output': 'Saída',
        'tooltip.period': 'Período',
        'tooltip.rate': 'Taxa',
        'tooltip.rtkTitle': 'RTK — Economia de tokens',
        'tooltip.saved': 'Economizado',
        'tooltip.scanningLocalHost': 'Lendo esta máquina — o uso no {remote} não é contabilizado.',
        'tooltip.settings': 'Configurações',
        'tooltip.settingsTitle': 'Abrir as configurações do otak-usage',
        'tooltip.thisMonth': 'Este mês',
        'tooltip.title': 'otak-usage — custo equivalente de API',
        'tooltip.today': 'Hoje',
        'tooltip.total': 'Total',
        'tooltip.updated': 'Atualizado',
    },
    ru: {
        'action.installOnRemote': 'Установить на {remote}',
        'action.notToday': 'Не сегодня',
        'action.openSettings': 'Открыть настройки',
        'alert.dailyCostExceeded': 'Ежедневное предупреждение otak-usage: сумма за сегодня {total}, что выше дневного порога {threshold}.',
        'alert.fastModeDetected': 'otak-usage: быстрый режим {provider} включён — использование тарифицируется по повышенным ценам быстрого режима.',
        'alert.fastModeOptimized': 'Оптимизация контекста была включена однократно, чтобы контекст в быстром режиме оставался компактным.',
        'alert.limitExceeded': 'Предупреждение о лимите otak-usage: использование {provider} {window} составляет {pct}%, что выше вашего порога предупреждения {threshold}%.',
        'alert.scanningLocalHost': 'otak-usage работает на этом компьютере и учитывает только локальное использование — то, что выполняется на {remote}, не включено.',
        'message.alertsResumed': 'otak-usage: предупреждения снова включены',
        'message.alertsSnoozed': 'otak-usage: предупреждения отключены до конца дня',
        'message.codexOptimizeApplied': 'otak-usage: оптимизация контекста Codex применена к config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: оптимизация контекста Codex удалена из config.toml',
        'message.summaryCopied': 'otak-usage: сводка скопирована в буфер обмена',
        'tooltip.allTime': 'За все время',
        'tooltip.clickToTogglePeriod': 'Нажмите, чтобы переключить вид',
        'tooltip.combinedTotal': 'Итого OpenAI + Claude',
        'tooltip.copySummary': 'Скопировать сводку',
        'tooltip.copySummaryTitle': 'Скопировать сводку использования в буфер обмена',
        'tooltip.input': 'Ввод',
        'tooltip.limitResets': 'сброс {time}',
        'tooltip.bankedResets': 'Сохранённые сбросы',
        'tooltip.limitUsed': '{pct}% использовано',
        'tooltip.limits': 'Лимиты',
        'tooltip.logDirectoryNotFound': 'Каталог журналов не найден.',
        'tooltip.model': 'Модель',
        'tooltip.noUsageThisMonth': 'Нет использования в этом месяце.',
        'tooltip.optimize': 'Оптимизировать',
        'tooltip.optimizeTitle': 'Настроить оптимизацию контекста Claude Code и Codex',
        'tooltip.output': 'Вывод',
        'tooltip.period': 'Период',
        'tooltip.rate': 'Доля',
        'tooltip.rtkTitle': 'RTK — Экономия токенов',
        'tooltip.saved': 'Сэкономлено',
        'tooltip.scanningLocalHost': 'Читает этот компьютер — использование на {remote} не учитывается.',
        'tooltip.settings': 'Настройки',
        'tooltip.settingsTitle': 'Открыть настройки otak-usage',
        'tooltip.thisMonth': 'Этот месяц',
        'tooltip.title': 'otak-usage — стоимость, эквивалентная API',
        'tooltip.today': 'Сегодня',
        'tooltip.total': 'Итого',
        'tooltip.updated': 'Обновлено',
    },
    tr: {
        'action.installOnRemote': '{remote} üzerine yükle',
        'action.notToday': 'Bugün değil',
        'action.openSettings': 'Ayarları Aç',
        'alert.dailyCostExceeded': 'otak-usage günlük maliyet uyarısı: bugünün toplamı {total}; günlük uyarı eşiği {threshold} üzerinde.',
        'alert.fastModeDetected': 'otak-usage: {provider} hızlı modu açık — kullanım, premium hızlı mod fiyatlarıyla faturalandırılır.',
        'alert.fastModeOptimized': 'Hızlı modda bağlamı kompakt tutmak için bağlam optimizasyonu bir defaya mahsus etkinleştirildi.',
        'alert.limitExceeded': 'otak-usage limit uyarısı: {provider} {window} kullanımı {pct}%, uyarı eşiğiniz olan {threshold}% üzerinde.',
        'alert.scanningLocalHost': 'otak-usage bu makinede çalışıyor, bu nedenle yalnızca yerel kullanımı sayar — {remote} üzerinde çalıştırdıklarınız dahil değildir.',
        'message.alertsResumed': 'otak-usage: uyarılar yeniden etkinleştirildi',
        'message.alertsSnoozed': 'otak-usage: uyarılar günün geri kalanında sessize alındı',
        'message.codexOptimizeApplied': 'otak-usage: Codex bağlam optimizasyonu config.toml dosyasına uygulandı',
        'message.codexOptimizeRemoved': 'otak-usage: Codex bağlam optimizasyonu config.toml dosyasından kaldırıldı',
        'message.summaryCopied': 'otak-usage: özet panoya kopyalandı',
        'tooltip.allTime': 'Tüm Zamanlar',
        'tooltip.clickToTogglePeriod': 'Görünümü değiştirmek için tıklayın',
        'tooltip.combinedTotal': 'OpenAI + Claude Toplamı',
        'tooltip.copySummary': 'Özeti Kopyala',
        'tooltip.copySummaryTitle': 'Kullanım özetini panoya kopyala',
        'tooltip.input': 'Girdi',
        'tooltip.limitResets': 'sıfırlama {time}',
        'tooltip.bankedResets': 'Birikmiş sıfırlamalar',
        'tooltip.limitUsed': '%{pct} kullanıldı',
        'tooltip.limits': 'Limitler',
        'tooltip.logDirectoryNotFound': 'Günlük dizini bulunamadı.',
        'tooltip.model': 'Model',
        'tooltip.noUsageThisMonth': 'Bu ay kullanım yok.',
        'tooltip.optimize': 'Optimize Et',
        'tooltip.optimizeTitle': 'Claude Code ve Codex bağlam optimizasyonunu yapılandırın',
        'tooltip.output': 'Çıktı',
        'tooltip.period': 'Dönem',
        'tooltip.rate': 'Oran',
        'tooltip.rtkTitle': 'RTK — Token Tasarrufu',
        'tooltip.saved': 'Tasarruf',
        'tooltip.scanningLocalHost': 'Bu makine okunuyor — {remote} üzerindeki kullanım sayılmıyor.',
        'tooltip.settings': 'Ayarlar',
        'tooltip.settingsTitle': 'otak-usage ayarlarını aç',
        'tooltip.thisMonth': 'Bu Ay',
        'tooltip.title': 'otak-usage — API eşdeğeri maliyet',
        'tooltip.today': 'Bugün',
        'tooltip.total': 'Toplam',
        'tooltip.updated': 'Güncellendi',
    },
    vi: {
        'action.installOnRemote': 'Cài đặt trên {remote}',
        'action.notToday': 'Hôm nay thì thôi',
        'action.openSettings': 'Mở cài đặt',
        'alert.dailyCostExceeded': 'Cảnh báo chi phí hằng ngày của otak-usage: tổng hôm nay là {total}, vượt ngưỡng cảnh báo hằng ngày {threshold}.',
        'alert.fastModeDetected': 'otak-usage: chế độ nhanh của {provider} đang bật — mức sử dụng được tính theo giá cao của chế độ nhanh.',
        'alert.fastModeOptimized': 'Tối ưu hóa ngữ cảnh đã được bật một lần để giữ ngữ cảnh gọn nhẹ trong chế độ nhanh.',
        'alert.limitExceeded': 'Cảnh báo giới hạn otak-usage: mức sử dụng {provider} {window} đang ở {pct}%, vượt ngưỡng cảnh báo {threshold}% của bạn.',
        'alert.scanningLocalHost': 'otak-usage đang chạy trên máy này nên chỉ tính mức sử dụng cục bộ — những gì bạn chạy trên {remote} không được tính.',
        'message.alertsResumed': 'otak-usage: đã bật lại cảnh báo',
        'message.alertsSnoozed': 'otak-usage: đã tắt cảnh báo trong phần còn lại của hôm nay',
        'message.codexOptimizeApplied': 'otak-usage: đã áp dụng tối ưu hóa ngữ cảnh Codex cho config.toml',
        'message.codexOptimizeRemoved': 'otak-usage: đã xóa tối ưu hóa ngữ cảnh Codex khỏi config.toml',
        'message.summaryCopied': 'otak-usage: đã sao chép tóm tắt vào clipboard',
        'tooltip.allTime': 'Từ trước đến nay',
        'tooltip.clickToTogglePeriod': 'Nhấp để đổi chế độ xem',
        'tooltip.combinedTotal': 'Tổng OpenAI + Claude',
        'tooltip.copySummary': 'Sao chép tóm tắt',
        'tooltip.copySummaryTitle': 'Sao chép tóm tắt sử dụng vào clipboard',
        'tooltip.input': 'Đầu vào',
        'tooltip.limitResets': 'đặt lại {time}',
        'tooltip.bankedResets': 'Reset đã lưu',
        'tooltip.limitUsed': 'đã dùng {pct}%',
        'tooltip.limits': 'Giới hạn',
        'tooltip.logDirectoryNotFound': 'Không tìm thấy thư mục nhật ký.',
        'tooltip.model': 'Mô hình',
        'tooltip.noUsageThisMonth': 'Không có sử dụng trong tháng này.',
        'tooltip.optimize': 'Tối ưu hóa',
        'tooltip.optimizeTitle': 'Cấu hình tối ưu hóa ngữ cảnh Claude Code và Codex',
        'tooltip.output': 'Đầu ra',
        'tooltip.period': 'Kỳ',
        'tooltip.rate': 'Tỷ lệ',
        'tooltip.rtkTitle': 'RTK — Tiết kiệm token',
        'tooltip.saved': 'Đã tiết kiệm',
        'tooltip.scanningLocalHost': 'Đang đọc máy này — mức sử dụng trên {remote} không được tính.',
        'tooltip.settings': 'Cài đặt',
        'tooltip.settingsTitle': 'Mở cài đặt otak-usage',
        'tooltip.thisMonth': 'Tháng này',
        'tooltip.title': 'otak-usage — chi phí tương đương API',
        'tooltip.today': 'Hôm nay',
        'tooltip.total': 'Tổng',
        'tooltip.updated': 'Đã cập nhật',
    },
    'zh-cn': {
        'action.installOnRemote': '安装到 {remote}',
        'action.notToday': '今天不再提醒',
        'action.openSettings': '打开设置',
        'alert.dailyCostExceeded': 'otak-usage 每日费用提醒：今天合计为 {total}，已超过每日提醒阈值 {threshold}。',
        'alert.fastModeDetected': 'otak-usage：{provider} 的 fast 模式已开启 — 使用量将按较高的 fast 模式费率计费。',
        'alert.fastModeOptimized': '已一次性启用上下文优化，以在 fast 模式下保持上下文精简。',
        'alert.limitExceeded': 'otak-usage 限额提醒：{provider} {window} 使用率为 {pct}%，已超过提醒阈值 {threshold}%。',
        'alert.scanningLocalHost': 'otak-usage 正在本机运行，因此只统计本地用量 — 你在 {remote} 上的用量不会计入。',
        'message.alertsResumed': 'otak-usage：已恢复提醒',
        'message.alertsSnoozed': 'otak-usage：今天剩余时间不再提醒',
        'message.codexOptimizeApplied': 'otak-usage：已将 Codex 上下文优化应用到 config.toml',
        'message.codexOptimizeRemoved': 'otak-usage：已从 config.toml 中移除 Codex 上下文优化',
        'message.summaryCopied': 'otak-usage：摘要已复制到剪贴板',
        'tooltip.allTime': '全部时间',
        'tooltip.clickToTogglePeriod': '点击切换视图',
        'tooltip.combinedTotal': 'OpenAI + Claude 合计',
        'tooltip.copySummary': '复制摘要',
        'tooltip.copySummaryTitle': '将使用量摘要复制到剪贴板',
        'tooltip.input': '输入',
        'tooltip.limitResets': '{time} 重置',
        'tooltip.bankedResets': '已储备重置',
        'tooltip.limitUsed': '已用 {pct}%',
        'tooltip.limits': '限额',
        'tooltip.logDirectoryNotFound': '未找到日志目录。',
        'tooltip.model': '模型',
        'tooltip.noUsageThisMonth': '本月无使用量。',
        'tooltip.optimize': '优化',
        'tooltip.optimizeTitle': '配置 Claude Code 和 Codex 上下文优化',
        'tooltip.output': '输出',
        'tooltip.period': '周期',
        'tooltip.rate': '比率',
        'tooltip.rtkTitle': 'RTK — Token 节省',
        'tooltip.saved': '节省',
        'tooltip.scanningLocalHost': '正在读取本机 — {remote} 上的用量未计入。',
        'tooltip.settings': '设置',
        'tooltip.settingsTitle': '打开 otak-usage 设置',
        'tooltip.thisMonth': '本月',
        'tooltip.title': 'otak-usage — API 等效费用',
        'tooltip.today': '今天',
        'tooltip.total': '合计',
        'tooltip.updated': '已更新',
    },
    'zh-tw': {
        'action.installOnRemote': '安裝到 {remote}',
        'action.notToday': '今天不再提醒',
        'action.openSettings': '開啟設定',
        'alert.dailyCostExceeded': 'otak-usage 每日費用提醒：今天合計為 {total}，已超過每日提醒門檻 {threshold}。',
        'alert.fastModeDetected': 'otak-usage：{provider} 的 fast 模式已開啟 — 使用量將按較高的 fast 模式費率計費。',
        'alert.fastModeOptimized': '已一次性啟用上下文最佳化，以在 fast 模式下保持上下文精簡。',
        'alert.limitExceeded': 'otak-usage 額度提醒：{provider} {window} 使用率為 {pct}%，已超過提醒門檻 {threshold}%。',
        'alert.scanningLocalHost': 'otak-usage 正在本機執行，因此只統計本機用量 — 你在 {remote} 上的用量不會計入。',
        'message.alertsResumed': 'otak-usage：已恢復提醒',
        'message.alertsSnoozed': 'otak-usage：今天剩餘時間不再提醒',
        'message.codexOptimizeApplied': 'otak-usage：已將 Codex 上下文最佳化套用至 config.toml',
        'message.codexOptimizeRemoved': 'otak-usage：已從 config.toml 中移除 Codex 上下文最佳化',
        'message.summaryCopied': 'otak-usage：摘要已複製到剪貼簿',
        'tooltip.allTime': '全部時間',
        'tooltip.clickToTogglePeriod': '點擊切換檢視',
        'tooltip.combinedTotal': 'OpenAI + Claude 合計',
        'tooltip.copySummary': '複製摘要',
        'tooltip.copySummaryTitle': '將使用量摘要複製到剪貼簿',
        'tooltip.input': '輸入',
        'tooltip.limitResets': '{time} 重置',
        'tooltip.bankedResets': '已儲備重設',
        'tooltip.limitUsed': '已用 {pct}%',
        'tooltip.limits': '限額',
        'tooltip.logDirectoryNotFound': '找不到記錄目錄。',
        'tooltip.model': '模型',
        'tooltip.noUsageThisMonth': '本月沒有使用量。',
        'tooltip.optimize': '最佳化',
        'tooltip.optimizeTitle': '設定 Claude Code 與 Codex 內容最佳化',
        'tooltip.output': '輸出',
        'tooltip.period': '期間',
        'tooltip.rate': '比率',
        'tooltip.rtkTitle': 'RTK — Token 節省',
        'tooltip.saved': '節省',
        'tooltip.scanningLocalHost': '正在讀取本機 — {remote} 上的用量未計入。',
        'tooltip.settings': '設定',
        'tooltip.settingsTitle': '開啟 otak-usage 設定',
        'tooltip.thisMonth': '本月',
        'tooltip.title': 'otak-usage — API 等效費用',
        'tooltip.today': '今天',
        'tooltip.total': '合計',
        'tooltip.updated': '已更新',
    },
};

export class I18n {
    private readonly locale: SupportedLocale;

    constructor(locale: string | undefined) {
        this.locale = resolveSupportedLocale(locale);
    }

    public t(key: TranslationKey, params?: Record<string, string>): string {
        const template = MESSAGES[this.locale][key] ?? MESSAGES.en[key];
        if (!params) {
            return template;
        }
        let result = template;
        for (const [name, value] of Object.entries(params)) {
            result = result.replaceAll(`{${name}}`, value);
        }
        return result;
    }

    public getCurrentLocale(): SupportedLocale {
        return this.locale;
    }
}

export function resolveSupportedLocale(locale: string | undefined): SupportedLocale {
    const normalized = (locale ?? '').trim().replace(/_/g, '-').toLowerCase();
    if (isSupportedLocale(normalized)) {
        return normalized;
    }

    const base = normalized.split('-')[0];
    if (base === 'zh') {
        return normalized.includes('hant') || normalized.includes('tw') || normalized.includes('hk') || normalized.includes('mo')
            ? 'zh-tw'
            : 'zh-cn';
    }
    if (base === 'pt') {
        return 'pt-br';
    }
    if (isSupportedLocale(base)) {
        return base;
    }
    return 'en';
}

function isSupportedLocale(locale: string): locale is SupportedLocale {
    return SUPPORTED_LOCALES.includes(locale as SupportedLocale);
}
