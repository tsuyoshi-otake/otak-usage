...
- **Anthropic usage API:** When `otakUsage.showRateLimits` is enabled (the default), the extension sends the locally stored Claude Code OAuth token only to `https://api.anthropic.com/api/oauth/usage` to retrieve subscription utilization.
- **ChatGPT usage API:** With the same setting enabled, the extension sends the locally stored Codex ChatGPT token only to `https://chatgpt.com/backend-api/wham/usage` to retrieve the banked rate-limit reset count. API-key-only Codex installs have no such token and make no request.
- Disable `otakUsage.showRateLimits` to prevent both usage requests.
